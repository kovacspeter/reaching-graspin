...
class TRPOAgent(Process):
    def __init__(self, env_name, name, max_kl, job_queue, result_queue):
        ...
        self.replay_buff = deque(maxlen=DEFAULT_REPLAY_MEMORY_SIZE)
    ...
    def learn(self, GAMMA, episodes, episode_length):
        ...
        new_trajs = self._collect_trajectories(episodes, episode_length)
        self.replay_buff.append(new_trajs)

        for trajs in self.replay_buff:
            for traj in trajs:
        ...
        # puts all the experiences in a matrix: DEFAULT_MEMORY_REPLAY_SIZE * total_timesteps x options
        action_mean = np.concatenate(
            [np.concatenate([traj["action_means"] for traj in trajs]) for trajs in self.replay_buff])
        action_logstd = np.concatenate(
            [np.concatenate([traj["action_stds"] for traj in trajs]) for trajs in self.replay_buff])
        obs_n = np.concatenate([np.concatenate([traj["obs"] for traj in trajs]) for trajs in self.replay_buff])
        action_n = np.concatenate([np.concatenate([traj["acts"] for traj in trajs]) for trajs in self.replay_buff])

        # standardize to mean 0 stddev 1
        advant_n = np.concatenate([np.concatenate([traj["advantage"] for traj in trajs]) for trajs in self.replay_buff])
