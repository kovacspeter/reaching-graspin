...
class RolloutProcess(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            # SET POLICY WEIGHTS
            elif job_id == 'update_params':
                self.trpo.sff(*job_data)
...
class ParallelRollout():
    ...
    def __init__(self, env_name, traj_len, n_processes, max_kl):
        ...
        for i in range(n_processes):
            proc = RolloutProcess(env_name, self.job_queue, self.result_queue, i, traj_len, max_kl)
...
class TRPOAgent(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            # job is array of format (job_id, job_data)
            job_id = job[0]
            job_data = job[1]
